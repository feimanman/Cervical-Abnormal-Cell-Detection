...
# IMAGE_FOLDER = "/mnt/data/feimanman/global_local_net/result/patch"
# IMAGE_FOLDER = "/mnt/data/feimanman/global_local_net/result/train/"

class PlantDataset(Dataset):
    """ Do normal training
    """

    def __init__(self, data,IMAGE_FOLDER, transforms=None):
        self.data = data
    ...
    def __getitem__(self, index):
        ...
        image = cv2.cvtColor(
            cv2.imread(os.path.join(self.IMAGE_FOLDER, self.data.iloc[index, 0] + ".jpg")), cv2.COLOR_BGR2RGB
        )
...
class PlantDataset1(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # solution-1: read from raw image
        image = cv2.cvtColor(
            cv2.imread(os.path.join(IMAGE_FOLDER, self.data[index, 0] + ".jpg")), cv2.COLOR_BGR2RGB
        )
        # solution-2: read from npy file which can speed the data load time.
        # image = np.load(os.path.join(NPY_FOLDER, "raw", self.data.iloc[index, 0] + ".npy"))
        ...
        #     )
        # else:
        label = torch.FloatTensor(self.data[index, 1:].values.astype(np.int64))
